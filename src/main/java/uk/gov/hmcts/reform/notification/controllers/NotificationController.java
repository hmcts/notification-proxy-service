package uk.gov.hmcts.reform.notification.controllers;
...
import uk.gov.hmcts.reform.notification.service.NotificationService;
...
public class NotificationController {
    ...
    public ResponseEntity emailNotification(
        ...
        log.info("recipientEmailAddress in request  for /email endpoint {}",request.getDestinationAddress());
        log.info("reference in request  for /email endpoint {}",request.getReference());
        notificationService.saveNotificationRequest(request, headers);
            return new ResponseEntity<>(
                    "Notification sent successfully via email", HttpStatus.CREATED);
    }

    @PostMapping("/notifications/scheduler")
